...
export type CliIo = { write(value: string): void | Promise<void> };

export async function runCli(argv: string[], io: CliIo = { write: writeStdout }): Promise<void> {
    const [command, ...args] = argv;
    if (command === 'routes') return printManifest(args, false, io);
    if (command === 'openapi') return printManifest(args, true, io);
...
async function generateProject(directory: string): Promise<void> {
    ...
    await writeIfMissing(join(root, 'package.json'), JSON.stringify({ type: 'module', scripts: { build: 'tsc', dev: 'tsx src/app.ts', routes: 'honolus routes --module ./dist/app.js' }, dependencies: { '@untitledsekai/honolus': '^1.0.1', hono: '^4.13.0' }, devDependencies: { typescript: '^5.0.0', tsx: '^4.0.0' } }, null, 2) + '\n');
...
async function printManifest(args: string[], openapi: boolean, io: CliIo): Promise<void> {
    ...
    if (output) await writeFile(resolve(output), text, 'utf8'); else await io.write(text);
}
...
if (require.main === module) {
    void runCli(process.argv.slice(2)).catch((error) => { console.error(error instanceof Error ? error.message : error); process.exitCode = 1; });
}
